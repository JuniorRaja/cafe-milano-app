...
- [x] If orphans exist, clean them in the v4→v5 migration **before** enabling the
...
- [x] `lib/database/app_database.dart` — add to `MigrationStrategy`:
      ```dart
...
- [x] Note the ordering constraint: the pragma must be **off** during the migration
      ...
      migration step.
- [x] Close the incomplete delete guards. Extend `shopHasOrders` /
...
- [x] Verify `backup_dao.dart:41-48` `restoreAll` still works with enforcement on.
...
- [x] Decide and record: no `ON DELETE CASCADE` anywhere. Deletion is blocked at
...
- [x] `lib/database/app_database.dart` — bump `schemaVersion` to `5`, add
...
- [x] Consider `daily_orders(order_date, shop_id)` as a composite instead of two
...
- [x] Verify with `EXPLAIN QUERY PLAN` on the heaviest dashboard query that the
      index is actually used. An index the planner ignores is pure write overhead.
- [x] `lib/services/backup_service.dart` — **no change needed** for indexes (they
...
- [x] `test/dao_test.dart` — deleting a shop with dependent `shop_prices` is
      rejected, not silently orphaned.
- [x] `test/backup_test.dart` — full export → wipe → import round-trip with
      enforcement on.

## Success criteria

- [x] `PRAGMA foreign_keys` reports `1` on a live connection.
- [ ] `PRAGMA foreign_key_check` returns zero rows on the migrated real database.
- [x] Attempting to delete a shop that has prices or standing orders is blocked in
...
- [x] `EXPLAIN QUERY PLAN` shows index usage on the revenue-for-date query.
